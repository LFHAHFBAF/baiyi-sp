...
-- 新增：手机按钮状态标记（记录是否点击开启治疗）
local mobileHealActive = false
...
local function setupMobileButton()
    ...
    healButton.MouseButton1Click:Connect(function()
        -- 第一次点击：开启治疗（等效电脑长按X）
        if not mobileHealActive then
            startSelfHeal()
            mobileHealActive = true
            healButton.BackgroundColor3 = Color3.fromRGB(255, 80, 80) -- 变红提示开启
            healButton.Text = "停止"
        -- 第二次点击：停止治疗（等效电脑松开X）
        else
            stopSelfHeal()
            mobileHealActive = false
            healButton.BackgroundColor3 = Color3.fromRGB(0, 150, 255) -- 变蓝提示关闭
            healButton.Text = "治疗"
        end
    end)

    print("手机治疗按钮加载成功（点击切换：治疗/停止）")
end
...
local function setupCharacter()
    ...
    Humanoid.Died:Connect(function()
        isSelfHealing = false
        mobileHealActive = false -- 重置手机按钮状态
